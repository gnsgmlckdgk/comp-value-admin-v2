 ...
 * - 검색 기능 (Ctrl+F)
 */
export default function LogStreamPopup() {
    ...
    const connectSSE = () => {
        try {
            // SSE 연결
            const eventSource = new EventSource('/dart/mgnt/stream');
            eventSourceRef.current = eventSource;

            eventSource.onopen = () => {
                setIsConnected(true);
                addLog('[시스템] 로그 스트림에 연결되었습니다.');
            };
            ...
            eventSource.onerror = (error) => {
                ...
                addLog('[오류] 로그 스트림 연결이 끊어졌습니다.');

                // 자동 재연결 시도 (3초 후)
                setTimeout(() => {
                    if (eventSourceRef.current?.readyState === EventSource.CLOSED) {
                        addLog('[시스템] 재연결을 시도합니다...');
                        connectSSE();
                    }
                }, 3000);
            };
    ...
    return (
        <div className="h-screen w-screen flex flex-col bg-slate-50 dark:bg-slate-900">
            {/* 헤더 */}
            <div className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-slate-200 dark:border-slate-700 bg-gradient-to-r from-blue-500 to-indigo-600 shadow-lg">
                    ...
                    <div className="flex items-center justify-center h-full">
                        <div className="text-center">
                            ...
                            <p className="text-sm text-slate-600">
                                {isConnected ? '로그를 기다리는 중...' : '연결을 시작하세요'}
                            </p>
                        </div>
                    </div>
                ) : (
                        ...
                        {logs.map((log, idx) => {
                            ...
                            return (
                                <div
                                    key={idx}
                                    id={`log-${idx}`}
                                    className={`text-sm leading-relaxed px-2 py-1 rounded transition-all ${
                                        isCurrentResult
            ...
            <div className="px-4 sm:px-6 py-2 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-lg">
                <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400">
                    <span>Endpoint: GET /dart/mgnt/stream | Ctrl+F: 검색</span>
