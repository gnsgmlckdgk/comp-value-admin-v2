...
export default function CointradeLogManagementModal({ onClose }) {
    ...
    const fetchFileList = async () => {
        ...
        setError('');
        try {
            const { data, error: apiError } = await send('/dart/api/cointrade/log/logs', {}, 'GET');
    ...
    const fetchLogContent = async (filename) => {
        ...
        setError('');
        try {
            const { data, error: apiError } = await send(`/dart/api/cointrade/log/logs/${filename}`, {}, 'GET');
    ...
    const openRealtimeLogStream = () => {
        ...
        window.open(
            '/member/cointrade-logstream',
            'Cointrade_Log_Stream',
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 dark:bg-black/60 px-4">
            <div className="w-full max-w-4xl max-h-[80vh] rounded-2xl bg-white dark:bg-slate-800 shadow-xl ring-1 ring-slate-900/5 dark:ring-slate-700 overflow-hidden flex flex-col">
                {/* 헤더 */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-gradient-to-r from-orange-500 to-red-600">
                    <h2 className="text-lg font-semibold text-white">코인 자동매매 프로그램 로그 관리</h2>
                    <button
                        onClick={onClose}
                    ...
                    {!loading && activeTab === 'logView' && (
                        <div className="space-y-4">
                            <select
                                ...
                                className="w-full px-4 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-orange-500"
                            >
                    ...
                    {!loading && activeTab === 'realtime' && (
                        <div className="text-center py-12">
                            ...
                            <button
                                ...
                                className="px-6 py-3 bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700 text-white font-medium rounded-lg shadow-lg transition-all transform hover:scale-105"
                            >
                                실시간 로그 추적 시작
                            </button>
                        </div>
