...
export default function LogManagementModal({ onClose }) {
    ...
    const fetchFileList = async () => {
        ...
        setError('');
        try {
            const { data, error: apiError } = await send('/dart/mgnt/logs', {}, 'GET');
    ...
    const fetchLogContent = async (filename) => {
        ...
        setError('');
        try {
            const { data, error: apiError } = await send(`/dart/mgnt/logs/${filename}`, {}, 'GET');
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 dark:bg-black/60 px-4">
            <div className="w-full max-w-4xl max-h-[80vh] rounded-2xl bg-white dark:bg-slate-800 shadow-xl ring-1 ring-slate-900/5 dark:ring-slate-700 overflow-hidden flex flex-col">
                {/* 헤더 */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-gradient-to-r from-blue-500 to-indigo-600">
                    <h2 className="text-lg font-semibold text-white">CompValue App 로그 관리</h2>
                    <button
                        onClick={onClose}
                    ...
                    {!loading && activeTab === 'realtime' && (
                        <div className="text-center py-12">
                            ...
                            <button
                                ...
                                className="px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-medium rounded-lg shadow-lg transition-all transform hover:scale-105"
                            >
                                실시간 로그 추적 시작
                            </button>
                        </div>
